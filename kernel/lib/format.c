...
#define FMT_SPACE (1 << 3)
#define FMT_SPECIAL (1 << 4)

#define FMT_TYPE_CHAR (1 << 5)
#define FMT_TYPE_STR (1 << 6)
#define FMT_TYPE_INT (1 << 7)

#define FMT_INT_B2 (1 << 8)
#define FMT_INT_B8 (1 << 9)
#define FMT_INT_B16 (1 << 10)
#define FMT_INT_SIGNED (1 << 11)
#define FMT_INT_UPPER (1 << 12)
...
static int get_arg(struct fmt* spec, struct va_wrap* va, char c, union arg* arg) {
	...
	case 'p':
		arg->p = va_arg(va->va, void*);
		spec->flags |= FMT_SPECIAL | FMT_TYPE_INT | FMT_INT_B16;
...
static int do_int(char* dest, char qualifier, unsigned long long x, unsigned int base, size_t dsize) {
	...
	return kulltostr(dest, x, base, dsize) == 0 ? 0 : -1;
}
...
static long long do_int_output(struct fmt* spec, char** dest, size_t* dsize, union arg* arg) {
	...
	int pad = 0;
	if (spec->field_width >= conversion_len + sign_len + radix_len) {
		spec->field_width -= conversion_len + sign_len + radix_len;
...
static inline long long do_output(struct fmt* spec, char** dest, size_t* dsize, union arg* arg) {
	int flag = spec->flags & (FMT_TYPE_CHAR | FMT_TYPE_STR | FMT_TYPE_INT);
	switch (flag) {
	case FMT_TYPE_CHAR:
